...
{
    public record GetTasksCurrentUserQuery : IRequest<GenericResponse<TasksDto>>
    {
        ...
        public class Handler(IApplicationDbContext context, ICurrentUserService currentUserService) : IRequestHandler<GetTasksCurrentUserQuery, GenericResponse<TasksDto>>
        {
            public async Task<GenericResponse<TasksDto>> Handle(GetTasksCurrentUserQuery request, CancellationToken cancellationToken)
            {
                var query = from t in context.DatTasks.AsNoTracking()
                            where !t.IsDeleted && t.Assigned == currentUserService.EmployeeNo

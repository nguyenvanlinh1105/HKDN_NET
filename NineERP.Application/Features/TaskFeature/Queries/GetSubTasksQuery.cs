...
{
    public record GetSubTasksQuery : IRequest<GenericResponse<TasksDto>>
    {
        ...
        public class Handler(IApplicationDbContext context) : IRequestHandler<GetSubTasksQuery, GenericResponse<TasksDto>>
        {
            public async Task<GenericResponse<TasksDto>> Handle(GetSubTasksQuery request, CancellationToken cancellationToken)
            {
                var query = from t in context.DatTasks.AsNoTracking()
                            where !t.IsDeleted && t.ParentId == request.ParentTaskId
                            ...
                            where !request.StartDate.HasValue || request.StartDate.Value.Date <= t.CreatedOn
                             && !request.EndDate.HasValue || request.EndDate.Value.Date >= t.CreatedOn
                            select new TaskDto
                            {
                                Id = t.Id,
